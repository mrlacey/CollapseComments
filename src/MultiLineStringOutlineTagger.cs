...
{
    public sealed class MultiLineStringOutlineTagger : ITagger<IOutliningRegionTag>
    {
        private readonly ITextBuffer buffer;
        private ITextSnapshot snapshot;
        private List<MultiLineStringRegion> regions;

        public MultiLineStringOutlineTagger(ITextBuffer buffer)
        {
            this.buffer = buffer;
            this.snapshot = buffer.CurrentSnapshot;
            this.regions = new List<MultiLineStringRegion>();
            this.ReParse();
        ...
        public IEnumerable<ITagSpan<IOutliningRegionTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            List<MultiLineStringRegion> currentRegions = this.regions;
        ...
        private static SnapshotSpan AsSnapshotSpan(MultiLineStringRegion region, ITextSnapshot snapshot)
        {
        ...
        private void ReParse()
        {
            ...
            var newRegions = new List<MultiLineStringRegion>();

            MultiLineStringRegion currentRegion = null;
            ...
            foreach (var line in newSnapshot.Lines)
            {
                ...
                if (startIndex < 0)
                {
                    startIndex = text.IndexOf("return \"");
                    startOffset = 7;
                }
                ...
                if (startIndex > 0 && endIndex < 0)
                {
                    ...
                    {
                        currentRegion = new MultiLineStringRegion
                        {
        ...
        private class MultiLineStringRegion
        {
